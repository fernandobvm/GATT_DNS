...
class Preprocessing:
    def __init__(self, domain, mesh, flow_type, flow_parameters, numMethods, casePath, p_row, p_col, logAll, tridimensional, time, runningLST = False):
        self.domain = domain
        self.mesh = mesh
    ...
    def prepare_sfd(self):
        if hasattr(self.numMethods, 'SFD'):
            ...
            if (self.numMethods.SFD.type > 0) and (os.path.exists(f"{self.casePath}/meanflowSFD.h5" or self.flow_type.initial_meanFile is not None)):
    ...
    def write_files(self):
        if not os.path.exists(self.casePath):
            os.mkdir(self.casePath)


        with h5py.File(f"{self.casePath}/mesh.h5", "w") as f:
            # Arrays diretos
            f.create_dataset("X", data=self.mesh.X, compression="gzip", compression_opts=9)
            f.create_dataset("Y", data=self.mesh.Y, compression="gzip", compression_opts=9)
            f.create_dataset("Z", data=self.mesh.Z, compression="gzip", compression_opts=9)
            f.create_dataset("wall", data=self.boundary.inside_wall, compression="gzip", compression_opts=9)

            if hasattr(self.mesh, "tracked_points"):
                f.create_dataset("tracked_points", data=self.mesh.tracked_points, compression="gzip", compression_opts=9)

            # flow_parameters: atributos numéricos simples
            flow_param_group = f.create_group("flow_parameters")
            self.save_object_attrs_as_group(flow_param_group, self.flow_parameters)

            # flow_type: atributos simples + vetores de objetos
            flow_type_group = f.create_group("flow_type")
            self.save_object_attrs_as_group(flow_type_group, self.flow_type)
                
        # Save mesh to a file using numpy
        #np.save(f"{self.casePath}/mesh.npy", {
        #    'X': self.mesh.X, 
        #    'Y': self.mesh.Y, 
        #    'Z': self.mesh.Z, 
        ...
        #np.savez(f"{self.casePath}/mesh.npz", X = self.mesh.X, Y = self.mesh.Y, Z = self.mesh.Z, wall = self.boundary.inside_wall, flow_parameters = self.flow_parameters, flow_type = self.flow_type)

        if self.runningLST == False:
            nStep, nx, ny, nz = checkPreviousRun(self.casePath)
        ...
        # Write Fortran files
        bin_path = f"{self.casePath}/bin"
        if not os.path.exists(bin_path):
            os.mkdir(bin_path)

        disturbTypes = self.writeFortranDisturbances(bin_path, self.boundaryInfo, self.tridimensional)
        self.writeFortranParameters(bin_path, self.mesh, self.flow_parameters, self.time, self.numMethods, self.logAll, self.p_row, self.p_col)
        self.writeFortranMatrices(bin_path, self.matrices, self.numMethods, self.mesh)
        self.writeFortranBoundaries(bin_path, self.boundaryInfo)

        # SFD
        if self.numMethods.SFD.type == 2:
            # Save in format .npy
            np.save(f"{bin_path}/SFD_X.npy", {'SFD_X': self.SFD_X})
            # Save in format .hdf5
            with h5py.File(f"{bin_path}/SFD_X.h5", 'w') as hdf5_file:
    ...
    def writeFortranDisturbances(self, bin_path, bi, tridimensional):
        """
        Creates runDisturbances.F90 and disturbances.F90 files
        """
        nProcs = len(bi)

        outFileDisturb = open(f'{bin_path}/runDisturbances.F90', 'w')
        if not tridimensional:
            outFileForcing = open(f'{bin_path}/runForcings2D.F90', 'w')
            open(f'{bin_path}/runForcings3D.F90', 'a').close()
        else:
            outFileForcing = open(f'{bin_path}/runForcings3D.F90', 'w')
            open(f'{bin_path}/runForcings2D.F90', 'a').close()
        ...
        outFile = open(f'{bin_path}/disturbances.F90', 'w')
    ...
    def writeFortranParameters(self, bin_path, mesh, flowParameters, time, numMethods, logAll, p_row, p_col):
        """
        This method writes the parameters.F90 file, which contains basic parameters that will be used in the simulation.
        """

        # Open the output file in write mode
        with open(f'{bin_path}/parameters.F90', 'w') as outFile:
            # Mesh parameters
            print('-------')
    ...
    def writeFortranMatrices(self, bin_path, matrices, num_methods, mesh):
        out_file = open(f'{bin_path}/matrices.F90', 'w')
        ...
        if not hasattr(mesh, 'tracked_points') or len(mesh.tracked_points) == 0:
            ...
            tracked_points = mesh.tracked_points
    ...
    def writeFortranBoundaries(self, bin_path, bi):
        """
        This method writes the boundaryInfo file with data for all boundary conditions for each domain slice.
        The file is saved in the bin_path directory in Fortran-compatible format.
        """
        nProcs = len(bi)
        ...
        with open(os.path.join(bin_path, 'boundaryInfo.F90'), 'w') as outFile:
        ...
        np.save(os.path.join(bin_path, 'boundary_conditions.npy'), bi)

    def save_object_attrs_as_group(self, h5group, obj):
        """Salva atributos de um objeto (inclusive aninhados) como datasets ou atributos HDF5."""
        for key, value in vars(obj).items():
            if isinstance(value, (list, tuple, np.ndarray)):
                # Checar se é vetor de objetos (ex: cav, disturb, rug)
                if len(value) > 0 and hasattr(value[0], '__dict__'):
                    subgrp = h5group.create_group(key)
                    for i, item in enumerate(value):
                        item_grp = subgrp.create_group(f"{i}")
                        self.save_object_attrs_as_group(item_grp, item)
                else:
                    value = np.array(value)
                    if value.ndim == 0:
                        h5group.create_dataset(key, data=value)
                    else:
                        h5group.create_dataset(key, data=value, compression="gzip", compression_opts=9)
                ...
                subgrp = h5group.create_group(key)
                self.save_object_attrs_as_group(subgrp, value)

            #else:
            #    print(f"Atributo ignorado: {key} (tipo {type(value)})")
