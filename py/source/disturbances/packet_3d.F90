    subroutine packet_3d(nx,ny,nz,x,y,z,t,u,omega0,nmodes,beta0,bmodes,amplitude)
    ...
    real*8 :: funcZ, funcT, envT, envX, envZ, normMode
    complex :: iImag
    ...
    if(t.gt.Tend) then
      ...
      do ix=1,nx
        ...
        do iz=1,nz
          envZ = dexp(-( z(iz)**2.d0) / (2.d0*sigmaZ**2.d0))

          do jmode=-bmode,bmode
            do imode=1,nmode

              funcZ = real(exp(iImag*jmode*beta0*z(iz)))
