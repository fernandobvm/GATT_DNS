    ...
    call readFlow(nSave,t,U,V,W,R,E, insideWall, NaN)
    ...
    if(SFD.gt.0) then
		...
		if(resumeMeanFlow.eq.1) then
            ...
            t2 = MPI_Wtime()
            ! Collective import via mpi/hdf5 asynchronous I/O
            call readMeanFlow(Umean,Vmean,Wmean,Rmean,Emean, insideWall, NaN)
            io_time = io_time + (MPI_Wtime() - t2)
		else
        ...
        if (stepsUntilSaving.eq.0) then
			...
			if(SFD.gt.0) then
				if(SFD_Delta.gt.0) then
                    ...
                    t3 = MPI_Wtime()
                    ! Collective export via mpi/hdf5 asynchronous I/O
                    call writeMeanFlow(Umean,Vmean,Wmean,Rmean,Emean, insideWall, NaN)
