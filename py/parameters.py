...
def parameters3():
    ...
    case_name = 'baseflow_gap_Re950_M01_h10_LR18_py'
    ...
    # Domain parameters
    domain = Domain(xi=-50, xf=500, yi=-0.1, yf=16, zi=0, zf=1)

    # Flow type - #TODO: blasiusFit,flowFile e meshFile
    flow_type = FlowType()
    flow_type.cav.append(Cavity([311.73, 329.91], [-0.1, 0], [-np.inf, np.inf]))
    ...
    mesh.y.attractor_regions = np.array([-2*D, -D/10, D/10, 2*delta99_end, 12])
